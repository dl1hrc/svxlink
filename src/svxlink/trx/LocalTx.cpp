...
bool LocalTx::initialize(void)
{
  ...
  */
  ...
  //Hier testen wir mal den Limiter*********************************************
  if (cfg.getValue(name(),"LIMITER",value) && (atoi(value.c_str()) !=0))
  {
  AudioCompressor *limit = new AudioCompressor;
  limit->setThreshold(-6);
  limit->setRatio(0.05);
  limit->setAttack(0.1);
  limit->setDecay(50);
...
//Hier mal ein Test mit dem Splatterfilter*************************************
string filterdesc;
if (cfg.getValue(name(),"TX_AUDIO_FILTER",filterdesc))
{
  AudioFilter *splatter_filter = new AudioFilter(filterdesc);
